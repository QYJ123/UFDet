...
class S2OBB2OBBDeltaXYSRTCoder(BaseBBoxCoder):
...
def obb2delta(proposals, gt, means=(0., 0., 0., 0., 0.), stds=(1., 1., 1., 1., 1.)):
    ...
    r = gw_regular/gh_regular*(gw_regular>=gh_regular)+gh_regular/gw_regular*(gw_regular<gh_regular)
    #dtheta = (gw_regular/gh_regular+gh_regular/gw_regular)*torch.sin(dtheta)
    dtheta = 0.5*r*torch.sin(2*dtheta)
    #dtheta = 0.5*(gw_regular/gh_regular+gh_regular/gw_regular)*dtheta
...
def delta2obb(proposals,
    ...
    #dtheta_sin = dtheta/(gw/gh+gh/gw)
    #dtheta_sin = dtheta_sin.clamp(min=-1, max=1)
    #gtheta = torch.asin(dtheta_sin) + ptheta
    
    r = gw/gh*(gw>=gh)+(gh/gw)*(gw<gh)
    dtheta_sin = 2*dtheta/r
    dtheta_sin = dtheta_sin.clamp(min=-1, max=1)
    gtheta = torch.asin(dtheta_sin)/2+ptheta
    
    #dtheta = 2*dtheta/(gw/gh+gh/gw)
