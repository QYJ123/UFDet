...
def obb2delta(proposals, gt, means=(0., 0., 0., 0., 0.), stds=(1., 1., 1., 1., 1.)):
    ...
    dtheta1 = torch.where(dtheta0>=pi/2,dtheta0-pi,(dtheta0+pi)*(dtheta0<-pi/2)+dtheta0*(dtheta0>=-pi/2)*(dtheta0<pi/2))

    gw_regular = torch.where(dtheta1.abs()<= pi/4, gw, gh)
    gh_regular = torch.where(dtheta1.abs()<= pi/4, gh, gw)
    dtheta_regular = torch.where(dtheta1.abs()<= pi/4, dtheta1, (dtheta1-pi/2)*(dtheta1>=0)+(dtheta1+pi/2)*(dtheta1<0))
...
def delta2obb(proposals,
    ...
    dL = dL0+(pw-ph)/(pw**2+ph**2).sqrt()
    da = da0+(pw-ph)/(pw**2+ph**2).sqrt()
    db1= db0-(pw+ph)/(pw**2+ph**2).sqrt()   
