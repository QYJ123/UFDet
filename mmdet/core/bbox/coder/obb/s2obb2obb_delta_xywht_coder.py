...
class S2OBB2OBBDeltaXYWHTCoder(BaseBBoxCoder):
...
def obb2delta(proposals, gt, means=(0., 0., 0., 0., 0.), stds=(1., 1., 1., 1., 1.)):
    ...
    da = dtheta*0.5*(gw_regular/gh_regular+gh_regular/gw_regular)
...
def delta2obb(proposals,
    ...
    gw = pw * dw.exp()
    gh = ph * dh.exp()
    delta_e = da*2/(gw/gh+gh/gw)
    
    gtheta = delta_e + ptheta
    gtheta = regular_theta(gtheta)
    gw_regular = torch.where(gw >= gh, gw, gh)
    gh_regular = torch.where(gw >= gh, gh, gw)
    gtheta_regular = torch.where(gw >= gh, gtheta,regular_theta(gtheta+pi/2))
