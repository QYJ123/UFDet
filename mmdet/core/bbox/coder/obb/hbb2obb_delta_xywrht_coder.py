...
class HBB2OBBDeltaXYWRHTCoder(BaseBBoxCoder):
...
def obb2delta(proposals, gt, theta_norm=True, means=(0., 0., 0., 0., 0.), stds=(1., 1., 1., 1., 1.)):
    ...
    dr = gw_regular/gh_regular

    if theta_norm:
        dtheta /= 2 * pi
    drtheta = dtheta*gw/gh
    deltas = torch.stack([dx, dy, dw, dr, drtheta], dim=-1)
...
def delta2obb(proposals,
    ...
    gw = pw * dw.exp()
    gh = gw/dr
    wh_ratio = torch.where(gw>gh,gw/gh,gh/gw)
    gtheta = drtheta/wh_ratio 
