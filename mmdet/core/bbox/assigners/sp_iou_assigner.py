...
def AspectRatio(gt_rbboxes):
    ...
    width  = gt_rbboxes[..., 2]
    height = gt_rbboxes[..., 3]
    ratios = (width / height)*(width>=height)+(height / width)*(width<height)
...
class SPIoUAssigner(BaseAssigner):
    ...
    def assign(self, bboxes, gt_bboxes, gt_bboxes_ignore=None, gt_labels=None):
        ...
        assign_result = self.assign_wrt_overlaps(gt_bboxes,overlaps,bboxes, gt_labels)
        if assign_on_cpu:
    ...
    def assign_wrt_overlaps(self,gt_bboxes,overlaps, bboxes, gt_labels=None):
        ...
        #k1 = (max_a_r-1)/max_a_r/(np.exp(s_value-f_value)-1)
        #iou_thr_weight = (1+(max_a_r-gt_bboxes_ratios[argmax_overlaps])/k1/max_a_r).log()+f_value
        #positive function
        ...
        if isinstance(self.neg_iou_thr, float):
            assigned_gt_inds[(max_overlaps >= 0)
                             & (max_overlaps <(self.neg_iou_thr*iou_thr_weight))] = 0
        

        # 3. assign positive: above positive IoU threshold
        pos_inds = (max_overlaps >= self.pos_iou_thr*iou_thr_weight)
