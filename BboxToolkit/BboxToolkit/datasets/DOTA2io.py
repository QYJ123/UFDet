...
def load_dota2(img_dir, ann_dir=None, classes=None, nproc=10):
    ...
    classes = get_classes('DOTA2' if classes is None else classes)
    cls2lbl = {cls: i for i, cls in enumerate(classes)}
    
    print('Starting loading DOTA2 dataset information.')
    start_time = time.time()
    _load_func = partial(_load_dota2_single,
                         img_dir=img_dir,
                         ann_dir=ann_dir,
                         cls2lbl=cls2lbl)
    if nproc > 1:
    ...
    end_time = time.time()
    print(f'Finishing loading DOTA2, get {len(contents)} iamges,',
          f'using {end_time-start_time:.3f}s.')

    return contents, classes


def _load_dota2_single(imgfile, img_dir, ann_dir, cls2lbl):
    ...
    content = _load_dota2_txt(txtfile, cls2lbl)

    content.update(dict(width=size[0], height=size[1], filename=imgfile, id=img_id))
    return content


def _load_dota2_txt(txtfile, cls2lbl):
...
def load_dota2_submission(ann_dir, img_dir=None, classes=None, nproc=10):
    classes = get_classes('DOTA2' if classes is None else classes)
    ...
    print('Starting loading DOTA2 submission information')
    start_time = time.time()
    infos_per_cls = []
    for cls in classes:
        if cls not in cls2file_mapper:
            ...
            infos_per_cls.append(_load_dota2_submission_txt(subfile))
    ...
    end_time = time.time()
    print(f'Finishing loading DOTA2 submission, get {len(contents)} images,',
          f'using {end_time-start_time:.3f}s.')
    return contents, classes


def _load_dota2_submission_txt(subfile):
...
def save_dota2_submission(save_dir, id_list, dets_list, task='Task1', classes=None, with_zipfile=True):
