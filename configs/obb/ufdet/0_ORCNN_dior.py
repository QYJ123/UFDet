...
test_cfg = dict(
    rpn=dict(
        ...
        min_bbox_size=0),
    rcnn=dict(
        score_thr=0.05, nms=dict(type='obb_nms', iou_thr=0.1), max_per_img=3000))
...
train_pipeline = [
    ...
    dict(type='RandomOBBRotate', rotate_after_flip=True, angles=(0, 0)),
...
data = dict(
    ...
    test=dict(
        ...
        img_prefix=data_root + 'JPEGImages-test/',
        pipeline=test_pipeline))
evaluation = None#dict(metric='mAP')
